#!/usr/bin/env python3

from functools import partial

from torch import Tensor
from botorch.sampling import SobolQMCNormalSampler

from albo.test_functions.synthetic import GardnerTestFunction
from albo.acquisition.objective import ClassicAugmentedLagrangianMCObjective
from albo.optim.optimize import AlboOptimizer


class Task():
    pass


class TaskGenerator(object):
    def __init__(self, inject_modules: object = None, conf: dict = None):
        self.inject_modules = inject_modules
        self.conf = conf

    def generate(self):
        for key, param in self.conf.items():

            task = Task()
            task.key = key
            task.param = param

            task.blackbox = GardnerTestFunction(noise_std=param.noise_std)
            task.bounds = Tensor(task.blackbox._bounds)

            task.objective = ClassicAugmentedLagrangianMCObjective(
                objective=lambda y: y[..., 0],
                constraints=list(lambda y, i=j: y[..., i] for j in range(1, task.blackbox.out_dim)),
                r=param['r']
            )

            task.sampler = SobolQMCNormalSampler(
                num_samples=param.num_mc_samples,
                seed=param.get('seed', None)
            )

            task.optimizer = AlboOptimizer(
                blackbox=task.blackbox,
                objective=task.objective,
                sampler=task.sampler,
                bounds=task.bounds
            )

            task.run = partial(
                task.optimizer.optimize,
                niter=param.num_iter,
                init_samples=param.num_init_samples,
                al_iter=param.num_al_iter,
                seed=param.get('seed', None)
            )

            yield task
